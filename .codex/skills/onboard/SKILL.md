...
`agent-loop` requires **both** the Codex and Claude CLIs. Its consumer-owned
config is `.codex/skills/agent-loop/agent-loop.config`.

The review hooks and Claude effort policy are pinned by the checked-in
launcher contract and reject consumer overrides. Do not edit them. Ask only
for the worker model and optional fallback when their keys are empty, using
exact identifiers supplied by the user or reported by the CLI:
...
If either required CLI is unavailable, say `agent-loop` cannot run; do not
change its pinned reviewer configuration.
