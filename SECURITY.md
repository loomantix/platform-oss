...
- `PHI_FIELD_NAMES` plus `CREDENTIAL_FIELD_NAMES` — together `SENSITIVE_FIELD_NAMES` — are the single source of truth for redaction. Both the stdout path (`formatters.log` plus a derived `redact.paths`) and the event-sink path (`logMetadata`) are built from that union, so a name added to either list protects both channels. Both match names ignoring case and `_`/`-`, so one entry covers `patientId`, `PatientId`, and `patient_id`. Before v0.4.0 the two lists were maintained separately and had drifted: clinical fields such as `transcript` and `soapNote` were stripped from the sink but written to stdout in cleartext.
- Redaction is **name-based**, applied at every depth. It is still best-effort: a sensitive value logged under a name not in the list, or interpolated into the `msg` string, is passed through. Use `assertPHISafe` in tests to catch this. The one exception is a URL: a string at a URL-shaped name (`url`, `uri`, `path`, `referer`, and similar) is parsed and its sensitive query-parameter values censored, because a secret inside a string cannot be reached by any name-matching rule. This covers URLs carried on an HTTP client's error, not just `req.url`.
...
- `redactS3Url` keeps only the file extension, and only when it is one of a known media/document allowlist — an identifier-shaped suffix such as `key.MRN9987` is dropped entirely. Object keys routinely carry patient names, MRNs, and dates of birth in the last path segment, so the filename is dropped rather than preserved.
