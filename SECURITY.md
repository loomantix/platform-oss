...
- `PHI_FIELD_NAMES` is the single source of truth for redaction. Both the stdout path (`formatters.log` plus a derived `redact.paths`) and the event-sink path (`logMetadata`) are built from it, so a name added there protects both channels. Both match names ignoring case and `_`/`-`, so one entry covers `patientId`, `PatientId`, and `patient_id`. Before v0.4.0 the two lists were maintained separately and had drifted: clinical fields such as `transcript` and `soapNote` were stripped from the sink but written to stdout in cleartext.
