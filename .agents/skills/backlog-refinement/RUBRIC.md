...
> **Versioned, reviewed criteria** for deciding whether a GitHub issue is _agent-intelligible_ — i.e. completable end-to-end by an autonomous Antigravity or Gemini CLI session inside `/agent-loop` without a human in the loop. This file is the single source of truth that both **`/backlog-refinement`** (which prepares the backlog) and **`/agent-loop`** (which consumes it) read. Every entry in [`LEARNINGS.md`](./LEARNINGS.md) either sharpens a _make-ready transformation_ or adds/tightens a _disqualifier_ here.
>
...
>
> **Rubric version: 2** — bump on every material change; record the bump in `LEARNINGS.md`. (v2: added the §2 _re-verify pre-tagged queue_ transformation.)
...
| **Re-verify pre-tagged queue** | A `dev: agent` issue lacks `agent: refined` | It was tagged by something other than this skill (older triage, bulk import, parallel pass) and never verified-against-HEAD. `refine --all` skips it (it's not "un-refined"), yet it's exactly what `/agent-loop` consumes. Run the full per-issue refine over the `candidates.py` **re-verify** bucket _before_ trusting the queue: strip `dev: agent` + add `agent-bail:` on failures, add `agent: refined` on passes. |
...
- **`agent-bail: epic`** — a tracking/coordination issue (`Epic:` title or cross-repo orchestration), not a bounded task.
...
Some issues are **opened _and_ closed by a scheduled workflow** — e.g. a nightly metrics/digest issue that the workflow itself auto-closes after N days. These are never engineering tasks, and a refinement comment on one resets its `updatedAt`, which can _delay_ that auto-close. Refinement must leave them alone: `candidates.py` routes any issue carrying an auto-managed label to a `skipped` bucket that never enters the work queue (and isn't counted as un-refined).

The label list is **repo-specific**, so declare it in the marker below (comma-separated). `candidates.py` reads exactly this marker from this file. Leave it empty if your repo has none.

<!-- auto-managed-labels: -->

<!-- ^ Fill in the marker above, e.g. `<!-- auto-managed-labels: nightly-digest, automated-report -->`. Keep it to a single marker line; the script reads the first match. -->
